...
function Ssr(userOptions: SsrOptionsType = {}): SsrType {
  ...
  function init(
    ...
    isSsr = emblaApiInstance.internalEngine().isSsr
    if (!isSsr) return

    emblaApi = emblaApiInstance
    isSsr = true
    coreOptions = emblaApi.internalEngine().options
  }
  ...
  function getNodes(options?: SsrOptionsType): NodesType {
    const slideSizes = (options && options.slideSizes) || []
    const rootSize = 100
  ...
  function createStyles(
    ...
    const { direction } = directionHandler.axis
    const { axis, location, slideLooper, contentSize } = createEngine(
      ssrOptions,
      container,
      slides
    )
